          ...
          <div className="mx-auto max-w-6xl space-y-6">
            <section className={surfaceStyles.hero}>
              ...
              <div className="mt-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
                ...
                <div className="flex flex-wrap gap-2">{detail.wallet ? <Link href={`/fidelite/${detail.wallet.token}`} target="_blank" className={buttonStyles.secondary}>Voir son portefeuille</Link> : null}<span className={detail.customer.opt_in_email ? badgeStyles.hans : badgeStyles.neutral}>{detail.customer.email_consent_source === "developer" ? "E-mail autorisé · mode dev" : detail.customer.opt_in_email ? "E-mail accepté" : "E-mail non accepté"}</span><span className={detail.customer.opt_in_sms ? badgeStyles.hans : badgeStyles.neutral}>{detail.customer.opt_in_sms ? "SMS accepté" : "SMS non accepté"}</span></div>
              </div>
            </section>
