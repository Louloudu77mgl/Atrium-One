...
export function AutomationsWorkspace({
  ...
  const selectedRun = currentAutomation?.executionHistory.find((run) => run.id === selectedRunId) ?? null;
  const runNodeIds = selectedRun?.steps.map((step) => step.nodeId) ?? [];
  const allHistory = useMemo(() => automations.flatMap((automation) => automation.executionHistory.map((run) => ({ ...run, automationTitle: automation.title }))).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()), [automations]);
  ...
  function openWorkflow(flow: AutomationFlow) {
    setSelectedAutomationId(flow.id);
    setSelectedNodeIds([]);
    ...
    <div className="mx-auto flex max-w-[1520px] flex-col gap-6 pb-16">
      <section className={`${shellCard} p-7`}>
        <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-4xl">
            ...
            <h1 className="mt-2 text-[34px] font-extrabold tracking-[-0.05em] text-[#17131F]">Une interface premium, claire et vendable dès aujourd’hui.</h1>
            <p className="mt-3 text-[15px] leading-7 text-[#6E6A76]">
              Trouvez une automatisation en quelques secondes, ouvrez un workflow sans être submergé et gardez un canvas grand, lisible et fluide.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => setCreateOpen(true)} className="rounded-[12px] bg-[#2B1A4A] px-5 py-3 text-sm font-semibold text-white">Nouvelle automatisation</button>
          </div>
        </div>
      </section>
          ...
          {listMode === "cards" ? (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {automations.map((automation) => (
                <button key={automation.id} type="button" onClick={() => openWorkflow(automation)} className="rounded-[24px] border border-[#EBE6DF] bg-[#FCFBF9] p-5 text-left transition hover:border-[#6E4DE0] hover:bg-[#FBF8FF]">
                  ...
                  <div className="mt-4 inline-flex rounded-[10px] bg-[#2B1A4A] px-4 py-2 text-sm font-semibold text-white">Ouvrir</div>
                </button>
              ))}
            </div>
          ) : (
        ...
        <div className="fixed inset-0 z-[80] flex flex-col overflow-hidden bg-[#F7F5F1]">
          ...
          <div className="hidden min-h-0 flex-1 flex-col lg:flex">
            <AutomationToolbar
              ...
              onAutoLayout={() => currentAutomation ? replaceCurrentAutomation(autoLayout(currentAutomation)) : undefined}
            ...
            <div className="flex min-h-0 flex-1 overflow-hidden">
              <div className="flex w-[320px] shrink-0 flex-col border-r border-[#E7E2DB] bg-white">
                ...
                <div className="shrink-0 border-t border-[#E7E2DB] p-4">
                  ...
                  <button type="button" onClick={() => setCreateOpen(true)} className="mt-2 w-full rounded-[10px] bg-[#2B1A4A] px-4 py-2.5 text-sm font-semibold text-white">
                    Ouvrir Hans
                  </button>
                </div>
              </div>
      ...
      {view === "history" ? (
        <section className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
          <AutomationHistory history={currentAutomation?.executionHistory ?? allHistory} selectedRunId={selectedRunId} onSelect={setSelectedRunId} />
          <div className={`${shellCard} p-6`}>
            <div className="text-[11px] font-bold uppercase tracking-[0.08em] text-[#9A96A1]">Détail</div>
            {selectedRun ? (
              <>
                <h2 className="mt-2 text-[22px] font-extrabold text-[#17131F]">{selectedRun.customerName}</h2>
                <p className="mt-1 text-[13px] leading-6 text-[#6E6A76]">{selectedRun.triggerLabel} · {formatDateTime(selectedRun.createdAt)} · {selectedRun.durationLabel}</p>
                <div className="mt-5 space-y-3">
                  {selectedRun.steps.map((step, index) => (
                    <div key={step.id} className="rounded-[20px] border border-[#EBE6DF] bg-[#FCFBF9] p-4">
                      <div className="text-[11px] font-bold uppercase tracking-[0.08em] text-[#9A96A1]">Bloc {index + 1}</div>
                      <div className="mt-1 text-[15px] font-extrabold text-[#17131F]">{step.title}</div>
              ...
              <div className="rounded-[20px] border border-dashed border-[#EBE6DF] bg-[#FCFBF9] p-5 text-[14px] leading-6 text-[#6E6A76]">
                Sélectionnez une exécution pour voir visuellement le chemin suivi.
              </div>
            )}
          </div>
        </section>
          ...
          <div className="mt-6 space-y-4">
            ...
            {mode === "template" ? (
              ...
              <div className="rounded-[22px] bg-[#F9F7F4] p-5">
                <div className="text-[14px] font-semibold text-[#17131F]">
                  {mode === "manual"
                    ? `Un canvas vide sera généré pour la catégorie ${theme.toLowerCase()}.`
                    : `Hans va générer un workflow pour la catégorie ${theme.toLowerCase()}.`}
                </div>
