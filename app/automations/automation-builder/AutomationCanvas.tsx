...
export function AutomationCanvas({
  ...
  return (
    <div
      ref={viewportRef}
      className={`relative cursor-grab touch-none overflow-hidden overscroll-contain bg-[linear-gradient(to_right,#EDE8F3_1px,transparent_1px),linear-gradient(to_bottom,#EDE8F3_1px,transparent_1px)] bg-[size:40px_40px] active:cursor-grabbing ${fillViewport ? "h-full min-h-0 rounded-none border-0" : "h-[78vh] min-h-[820px] rounded-[32px] border border-[#D8D1E4]"}`}
      onPointerDown={(event) => {
      ...
      onWheel={(event) => {
        ...
        onPanChange({
          x: pan.x - (event.shiftKey && event.deltaX === 0 ? event.deltaY : event.deltaX),
    ...
    >
      <div className="absolute left-5 top-5 z-10 rounded-full bg-white/95 px-4 py-2 text-[12px] font-semibold text-[#6E6A76] shadow">Molette pour naviguer · Ctrl/Cmd + molette pour zoomer · glisser pour déplacer</div>
