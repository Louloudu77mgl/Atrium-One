...
export async function POST(request: Request) {
  ...
  try {
    ...
    let preview: EmailCampaignContent | null = null;
    if (payload?.content) {
      try {
        preview = normalizeEmailContent(payload.content);
        const error = emailContentError(preview);
    ...
    await sendGmailMessage({
      ...
      html: preview ? renderEmailHtml({
        campaign: { id: "preview", merchant_id: merchant.id, content: preview }, merchant,
        recipient: { id: "preview", token: "", email: connection.gmail_address, firstName: "Marie", lastName: "" }
      }) : renderConnectionTest(connection.gmail_address)
    });
