      ...
      {createOpen ? (
        <section id="rcu-editor" className={surfaceStyles.section}>
          ...
          {advancedOptionsOpen ? <div className="mt-4 grid gap-4 border-t border-[var(--color-border)] pt-4">
            ...
            <div className="grid gap-3 md:grid-cols-2"><label className="text-xs font-bold text-[var(--color-text-muted)]">Titre de l’affiche<input value={posterHeadline} maxLength={76} onChange={(event) => setPosterHeadline(event.target.value)} className="ao-input ao-focus mt-1 w-full px-3.5 py-2.5 text-sm" /></label><label className="text-xs font-bold text-[var(--color-text-muted)]">Message après inscription<input value={successMessage} onChange={(event) => setSuccessMessage(event.target.value)} className="ao-input ao-focus mt-1 w-full px-3.5 py-2.5 text-sm" /></label></div>
            <label className="text-xs font-bold text-[var(--color-text-muted)]">Texte de l’affiche<textarea value={posterBody} maxLength={200} onChange={(event) => setPosterBody(event.target.value)} rows={3} className="ao-input ao-focus mt-1 w-full resize-none px-3.5 py-2.5 text-sm" /></label>
          </div> : null}
