        ...
        {EMAIL_SEGMENT_DEFINITIONS.map((definition) => {
          ...
          return (
            <div key={definition.id} className={`ao-card-subtle p-4 ${selected ? "border-[var(--color-primary-hover)] bg-[var(--color-primary-muted)] shadow-sm" : ""}`}>
              ...
              {selected && definition.valueLabel ? <label className="mt-3 flex items-center gap-2 border-t border-[var(--color-border)] pt-3 text-xs font-bold text-[var(--color-text-muted)]"><input type="number" min={1} max={365} value={selected.value ?? definition.defaultValue} onChange={(event) => updateValue(definition.id, Number(event.target.value))} className={`${fieldStyles.input} h-8 w-20 px-2 py-1.5`} />{definition.valueLabel}</label> : null}
            </div>
          );
        })}
      </div>
