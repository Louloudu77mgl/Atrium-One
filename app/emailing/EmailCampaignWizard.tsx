...
import { EMAIL_CAMPAIGN_TYPE_OPTIONS, type EmailCampaignContent, type EmailCampaignRecord, type EmailCampaignType, type EmailSegmentMode, type EmailSegmentRule, type EmailSubscriberProfile } from "@/lib/emailing-types";
...
export function EmailCampaignWizard({
  ...
  async function generate() {
    ...
    try {
      ...
      const payload = await response.json() as { content?: EmailCampaignContent; error?: string; notice?: string };
      if (!response.ok || !payload.content) throw new Error(payload.error || "Hans n’a pas pu préparer l’e-mail.");
      setContent({ ...payload.content, editorMode: payload.content.editorMode ?? "visual" });
      setGenerationNotice(payload.notice || "");
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-[#211432]/60 px-3 py-4 backdrop-blur-sm sm:px-6 sm:py-8">
      <div className="mx-auto w-full max-w-6xl overflow-hidden rounded-[28px] bg-white shadow-[0_28px_90px_rgba(33,20,50,0.4)]">
        ...
        <div className="max-h-[calc(100vh-220px)] overflow-y-auto px-5 py-6 sm:px-7">
          ...
          {step === 3 ? <section className="mx-auto max-w-3xl"><div className="flex items-center gap-4"><HansAvatar size={58} /><div><div className="text-xs font-black uppercase tracking-[0.12em] text-[#7C3AED]">Parlez à Hans</div><h3 className="mt-1 text-2xl font-black text-[#211432]">Que souhaitez-vous dire ?</h3></div></div><div className="mt-6 rounded-[24px] border border-[#D8C9F1] bg-gradient-to-br from-[#FBF9FF] to-[#F3E8FF] p-5"><textarea autoFocus disabled={generating} rows={8} value={brief} onChange={(event) => setBrief(event.target.value)} placeholder="Ex. Écris un e-mail chaleureux annonçant notre nouvelle collection de pains d’été. Mets en avant les recettes légères et invite les clients à venir les goûter cette semaine." className="w-full resize-y bg-transparent text-base font-medium leading-7 text-[#211432] outline-none placeholder:text-[#9B91A8]" /><div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-[#DCCEF2] pt-4"><div className="text-xs font-semibold text-[#736A80]">Hans rédige votre message et génère un nouveau visuel pour cette campagne. Comptez une à deux minutes.</div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setStep(4)} disabled={generating} className={buttonStyles.tertiary}>Importer HTML / créer manuellement</button><button type="button" onClick={() => void generate()} disabled={generating} className={`${buttonStyles.primary} gap-2 disabled:opacity-60`}><Icon name="sparkle" className="h-4 w-4" />{generating ? "Hans crée le texte et le visuel…" : "Générer avec Hans"}</button></div></div></div><div className="mt-4 flex flex-wrap gap-2">{["Réactiver les clients absents depuis un mois", "Présenter notre nouveauté de saison", "Remercier nos clients les plus fidèles"].map((example) => <button type="button" key={example} onClick={() => setBrief(example)} className="rounded-full border border-[#E1D8EC] bg-white px-3 py-2 text-xs font-bold text-[#6B617F] hover:border-[#A78BFA]">{example}</button>)}</div></section> : null}
          ...
          {generationNotice ? <div role="status" className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">{generationNotice}</div> : null}
