    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-[#211432]/60 px-3 py-4 backdrop-blur-sm sm:px-6 sm:py-8">
      <div className="mx-auto w-full max-w-6xl overflow-hidden rounded-[28px] bg-white shadow-[0_28px_90px_rgba(33,20,50,0.4)]">
        ...
        <div className="max-h-[calc(100vh-220px)] overflow-y-auto px-5 py-6 sm:px-7">
          ...
          {step === 4 ? <section><div className="mb-5"><h3 className="text-2xl font-black text-[#211432]">Votre campagne est prête</h3><p className="mt-1 text-sm font-medium text-[#736A80]">Modifiez ce que vous voulez, puis choisissez quand l’envoyer à {audience.length} abonné{audience.length > 1 ? "s" : ""}.</p></div><EmailEditor content={content} onChange={setContent} merchant={merchant} sampleSubscriber={audience[0]} /><div className="mt-6 rounded-[22px] border border-[#E1D8EC] bg-[#FBFAFD] p-4"><div className="grid gap-4 lg:grid-cols-[1fr_auto]"><label className="grid gap-1.5 text-xs font-black text-[#51485F]">Programmer une date<input type="datetime-local" value={scheduledAt} onChange={(event) => setScheduledAt(event.target.value)} className="max-w-sm rounded-xl border border-[#DED7E8] bg-white px-3 py-2.5 text-sm outline-none focus:border-[#7C3AED]" /></label><div className="flex flex-wrap items-end gap-2"><button type="button" onClick={() => void sendTest()} disabled={testSending || !providerReady} className={`${buttonStyles.tertiary} disabled:cursor-not-allowed disabled:opacity-45`}>{testSending ? "Envoi du test…" : "M’envoyer un test"}</button><button type="button" onClick={() => void save("draft")} disabled={saving} className={buttonStyles.secondary}>Enregistrer comme brouillon</button><button type="button" onClick={() => void save("scheduled")} disabled={saving || !scheduledAt || !providerReady || audience.length === 0} className={`${buttonStyles.secondary} disabled:cursor-not-allowed disabled:opacity-45`}>Programmer</button><button type="button" onClick={() => void save("send")} disabled={saving || !providerReady || audience.length === 0} className={`${buttonStyles.primary} gap-2 disabled:cursor-not-allowed disabled:opacity-45`}><Icon name="mail" className="h-4 w-4" />{saving ? "Traitement…" : "Envoyer maintenant"}</button></div></div>{!providerReady ? <p className="mt-3 rounded-xl bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800">Connectez Gmail pour programmer ou envoyer réellement cette campagne. Le brouillon peut déjà être enregistré.</p> : audience.length === 0 ? <p className="mt-3 rounded-xl bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800">Aucun client n’a encore accepté les e-mails commerciaux. Vous pouvez vous envoyer un test, mais une campagne client restera bloquée tant qu’aucun consentement n’est enregistré.</p> : null}{testNotice ? <p className="mt-3 rounded-xl bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-800">✓ {testNotice}</p> : null}</div></section> : null}
