...
export function LeadsWorkspace({ initialLeads }: { initialLeads: LeadRow[] }) {
  ...
  const visible = useMemo(() => {
    ...
    const rows = leads.filter((lead) => {
      ...
      return (!normalized || haystack.includes(normalized)) && (!filters.city || lead.city === filters.city) && (!filters.businessType || lead.business_type === filters.businessType) && (!filters.status || lead.commercial_status === filters.status)
        ...
        && has(lead.phone, filters.phone) && has(lead.email, filters.email) && has(lead.website, filters.website) && has(lead.business_id || lead.auth_user_id, filters.account)
    ...
    return rows.sort((a, b) => {
      ...
      if (sort === "appointment") return (a.next_event?.event_date ?? "9999").localeCompare(b.next_event?.event_date ?? "9999");
  ...
  async function runBulk(action: "archive" | "delete", ids = [...selected]) {
    if (!ids.length) return;
    if (action === "delete" && !confirm(`Supprimer ${ids.length} prospect${ids.length > 1 ? "s" : ""} ?\n\nLes comptes AtriumOne associés seront conservés.`)) return;
    if (action === "archive" && !confirm(`Archiver ${ids.length} prospect${ids.length > 1 ? "s" : ""} ?`)) return;
    setBusy(true);
    const response = await fetch("/api/crm/leads/bulk", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, leadIds: ids }) });
    const data = await response.json();
    if (response.ok) { const removed = new Set(ids); setLeads((current) => current.filter((lead) => !removed.has(lead.id))); setSelected(new Set()); setNotice(`${data.updated} prospect${data.updated > 1 ? "s" : ""} ${action === "archive" ? "archivé" : "supprimé"}${data.updated > 1 ? "s" : ""}.`); }
    setBusy(false);
  }

  async function createBulkTasks(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy(true);
    const form = Object.fromEntries(new FormData(event.currentTarget).entries());
    const response = await fetch("/api/crm/leads/bulk", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "task", leadIds: [...selected], type: form.type, dueDate: form.dueDate, dueTime: form.dueTime, description: form.description }) });
    const data = await response.json();
    if (response.ok) { setNotice(`${data.created} tâches créées`); setShowBulkTask(false); setSelected(new Set()); router.refresh(); }
    setBusy(false);
  }
  ...
  return <div className="space-y-4 p-5 lg:p-8">
    ...
    <div className="overflow-hidden rounded-xl border border-[#E8E4DB] bg-white shadow-sm"><div className="border-b border-[#E8E4DB] px-4 py-3 text-xs font-bold text-[#6B617F]">{visible.length} résultat{visible.length > 1 ? "s" : ""}</div><div className="overflow-x-auto"><table className="min-w-[1390px] w-full text-left text-xs"><thead className="bg-[#FAF9F7] text-[10px] uppercase tracking-wide text-[#8B7AA8]"><tr><th className="p-3"><input aria-label="Tout sélectionner" type="checkbox" checked={allVisibleSelected} onChange={() => setSelected(allVisibleSelected ? new Set() : new Set(visible.map((lead) => lead.id)))} /></th>{["Nom", "Ville", "Métier", "Téléphone", "Email", "Avis", "Note", "Statut", "Compte AtriumOne", "Prochaine tâche", "Prochain RDV", "Dernière interaction", "Actions"].map((label) => <th key={label} className="p-3 font-black">{label}</th>)}</tr></thead><tbody>{visible.map((lead) => <tr key={lead.id} className="border-t border-[#EEEAE3] hover:bg-[#FCFBF9]"><td className="p-3"><input aria-label={`Sélectionner ${lead.name}`} type="checkbox" checked={selected.has(lead.id)} onChange={() => toggle(lead.id)} /></td><td className="p-3"><Link href={`/crm/leads/${lead.id}`} className="font-black hover:text-[#4C1D95]">{lead.name}</Link><div className="mt-0.5 text-[10px] text-[#8B7AA8]">{lead.lead_source}</div></td><td className="p-3 font-semibold">{lead.city ?? "—"}</td><td className="p-3">{lead.business_type ?? "—"}</td><td className="p-3">{lead.phone ?? "—"}</td><td className="max-w-[170px] truncate p-3">{lead.email ?? "—"}</td><td className="p-3">{lead.google_reviews_count ?? "—"}</td><td className="p-3 font-black">{lead.google_rating ? `${lead.google_rating} ★` : "—"}</td><td className="p-2"><select aria-label={`Statut de ${lead.name}`} value={lead.commercial_status} disabled={busyId === lead.id} onChange={(event) => void updateLead(lead.id, { commercial_status: event.target.value })} className="ao-select h-8 min-w-[135px] px-2 text-[11px] font-black">{COMMERCIAL_STATUSES.map((status) => <option key={status}>{status}</option>)}</select></td><td className="p-3"><Link href={`/crm/leads/${lead.id}?tab=access`} className={`rounded-full px-2 py-1 text-[10px] font-black ${lead.account_access?.account_enabled ? "bg-emerald-50 text-emerald-700" : lead.business_id ? "bg-amber-50 text-amber-700" : "bg-gray-100 text-gray-600"}`}>{lead.account_access?.account_enabled ? "Actif" : lead.business_id ? "En attente" : lead.auth_user_id ? "Inscrit · à préparer" : "Aucun compte"}</Link></td><td className="p-3">{lead.next_task ? <>{dateLabel(lead.next_task.due_date)}<div className="text-[10px] text-[#8B7AA8]">{lead.next_task.title}</div></> : "—"}</td><td className="p-3">{lead.next_event ? <>{dateLabel(lead.next_event.event_date)} · {lead.next_event.event_time?.slice(0,5) ?? "—"}<div className="text-[10px] font-black text-[#7C3AED]">{lead.next_event.type}</div></> : "—"}</td><td className="p-3">{dateLabel(lead.last_interaction)}</td><td className="p-3"><div className="flex gap-2"><Link href={`/crm/leads/${lead.id}`} className="text-[11px] font-black text-[#4C1D95]">Ouvrir la fiche</Link><button onClick={() => void runBulk("archive", [lead.id])} className="text-[11px] font-black text-[#8B7AA8]">Archiver</button><button onClick={() => void runBulk("delete", [lead.id])} className="text-[11px] font-black text-red-600">Supprimer</button></div></td></tr>)}</tbody></table>{!visible.length ? <div className="p-10 text-center text-sm font-semibold text-[#8B7AA8]">Aucun prospect ne correspond à ces filtres.</div> : null}</div></div>

    {showBulkTask ? <div className="ao-modal-backdrop"><form onSubmit={createBulkTasks} className="ao-modal-content max-w-lg p-5"><div className="flex justify-between"><div><h2 className="text-lg font-black">Ajouter une tâche</h2><p className="text-xs font-semibold text-[#8B7AA8]">Une tâche distincte sera créée pour chacun des {selected.size} prospects.</p></div><button type="button" onClick={() => setShowBulkTask(false)}>✕</button></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="ao-label">Type<select name="type" className="ao-select h-10 px-3"><option>Appel</option><option>Email</option></select></label><label className="ao-label">Date<input required type="date" name="dueDate" className="ao-input h-10 px-3" /></label><label className="ao-label">Heure facultative<input type="time" name="dueTime" className="ao-input h-10 px-3" /></label><label className="ao-label sm:col-span-2">Description facultative<textarea name="description" className="ao-input min-h-20 px-3 py-2" /></label></div><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setShowBulkTask(false)} className="ao-btn-secondary px-4 py-2 text-xs font-black">Annuler</button><button disabled={busy} className="ao-btn-primary px-4 py-2 text-xs font-black">Créer {selected.size} tâches</button></div></form></div> : null}
