...
type SavedSearch = { id: string; city: string; business_type: string; result_count: number; imported_count: number; results: PlacesProspect[]; next_page_token: string | null; created_at: string };
...
export function ProspectionWorkspace({ initialSearches }: { initialSearches: SavedSearch[] }) {
  ...
  const [busy, setBusy] = useState<"search" | "more" | "import" | null>(null);
  ...
  async function runSearch(loadMore = false, overrides?: { city: string; businessType: string }) {
    ...
    setBusy(loadMore ? "more" : "search"); setError(null);
    try {
      ...
      setCity(nextCity); setBusinessType(nextType); setResults((current) => loadMore ? [...current, ...data.prospects] : data.prospects);
      setSearchId(data.searchId); setNextPageToken(data.nextPageToken); if (!loadMore) setSelected(new Set());
      if (!loadMore) {
        setSearches((current) => [{ id: data.searchId, city: nextCity, business_type: nextType, result_count: data.prospects.length, imported_count: 0, results: data.prospects, next_page_token: data.nextPageToken, created_at: new Date().toISOString() }, ...current]);
      }
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Recherche impossible."); } finally { setBusy(null); }
  }

  function openSearch(search: SavedSearch) {
    setCity(search.city); setBusinessType(search.business_type); setResults(search.results ?? []); setSearchId(search.id); setNextPageToken(search.next_page_token); setSelected(new Set()); setError(null);
  }

  async function deleteSearch(id: string) {
    if (!confirm("Supprimer cette recherche de la bibliothèque ?")) return;
    const response = await fetch(`/api/crm/searches/${id}`, { method: "DELETE" });
    if (response.ok) { setSearches((current) => current.filter((item) => item.id !== id)); if (searchId === id) { setResults([]); setSearchId(null); } }
  }
  ...
  async function importSelection(ids?: string[]) {
    ...
    try {
      ...
      setSelected(new Set());
      setSearches((current) => current.map((item) => item.id === searchId ? { ...item, imported_count: item.imported_count + (data.imported?.length ?? 0) } : item));
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Import impossible."); } finally { setBusy(null); }
  }
    ...
    <div className="space-y-5 p-5 lg:p-8">
      ...
      <section>
        ...
        {searches.length ? <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{searches.map((search) => <article key={search.id} className="rounded-xl border border-[#E8E4DB] bg-white p-3">
          ...
          <div className="mt-3 flex gap-3 text-[11px] font-bold text-[#6B617F]"><span>{search.result_count} résultats</span><span>{search.imported_count} ajoutés</span></div>
          <div className="mt-3 flex gap-1.5"><button onClick={() => openSearch(search)} className="rounded-md bg-[#F3E8FF] px-2.5 py-1.5 text-[11px] font-black text-[#4C1D95]">Ouvrir</button><button onClick={() => void runSearch(false, { city: search.city, businessType: search.business_type })} className="rounded-md border border-[#E8E4DB] px-2.5 py-1.5 text-[11px] font-bold">Relancer</button><button onClick={() => void deleteSearch(search.id)} className="ml-auto px-2 py-1 text-[11px] font-bold text-red-600">Supprimer</button></div>
